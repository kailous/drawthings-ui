...
3. 脚本用默认参数构造 payload，并写入 `studio/payload.json`，方便 Web UI 后续查看或复用。
4. 脚本直接 POST 到 Draw Things HTTP API，不经过 `webui/` 页面。
5. 如果 API 返回 base64 图片，脚本保存到 `studio/direct_outputs/`。
...
```bash
python3 .skill/direct_generate.py --payload-json studio/payload.json
```
...
- 当前 payload: `studio/payload.json`
- 生成图片: `studio/direct_outputs/*.png`
- 响应摘要: `studio/direct_outputs/*_response.json`
