...
class ImageViewer {
    ...
    navigate(dir) {
        ...
        if (nextIdx < 0 || nextIdx >= this.data.length) {
            ...
            if (typeof this.config.onReachEdge === 'function') {
                this.config.onReachEdge(dir);
            }
            return;
        }
    ...
    notifyImageChange() {
        if (typeof this.config.onImageChange === 'function') {
            this.config.onImageChange(this.currentItem(), this.currentIndex);
        }
    }
